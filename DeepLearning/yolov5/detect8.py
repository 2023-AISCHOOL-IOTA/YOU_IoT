...
def run(
    ...
    # Directories
    # exp 번호가 증가하지 않고 exp 폴더에 계속해서 저장됨
...
#                 name_list = []
        ...
        if name_list == []:
            isCheck = False
#             os.system('sudo python neo_green.py')
        else:
            if 'cargo' in name_list:
                print("15분간 주차 가능합니다")
                mylcd.lcd_display_string("Available for", 1, 0)
                mylcd.lcd_display_string("15 minutes", 2, 0)
#                 os.system('sudo python neo_blue.py')
#                 os.system('sudo python neo_red.py')
                os.system('sudo python3 neo_blue.py')
                for i in range(0,3):
                    buzzer()
                    time.sleep(0.5)
            ...
            elif 'car' in name_list:
                print("허용 시간 내에만 이용바랍니다")
                mylcd.lcd_display_string("Allowed", 1, 0)
                mylcd.lcd_display_string("time only", 2, 0)
                time.sleep(10)
            isCheck = True
        mylcd.lcd_clear()
...
if __name__ == '__main__':
        ...
        #os.system()은 terminal에서 명령어 실행
        #data/test 경로에 test.png 파일로 저장       
        
    os.system('libcamera-jpeg -o data/test/test.png')     
    opt = parse_opt()
    main(opt)
